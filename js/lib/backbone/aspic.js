...
Backbone.AspicView = Backbone.View.extend({
	...
	*/		
    ...
    unbindFromField: function (selector) {
        ...
        for (var i = 0, ii = binded.length; i < ii; i++) {
            binded[i].model.off(binded[i].eventType, binded[i].handler);
        }

        // unsubscribe handlers from view
        binded = this._aspic_.binded[selector].bindedToView;
        for (var i = 0, ii = binded.length; i < ii; i++) {
            this.$el.off(binded[i].eventTypes, binded[i].selector, binded[i].handler);
        }
	...
	*/	
    ...
    _findPropertyByPath_: function (pathString, ctxModel) {
        ctxModel = ctxModel || this.model;
        var paths = pathString.split('.');
        switch (paths.length) {
            case 1: return { model: ctxModel, prop: paths[0] };
            case 2: return { model: ctxModel.get(paths[0]), prop: paths[1] };
            case 3: return { model: ctxModel.get(paths[0]).get(paths[1]), prop: paths[2] };
            case 4: return { model: ctxModel.get(paths[0]).get(paths[1]).get(paths[2]), prop: paths[3] };
            case 5: return { model: ctxModel.get(paths[0]).get(paths[1]).get(paths[2]).get(paths[3]), prop: paths[4] };
    ...
    _twoWayBindingPropertyAndModel_: function ($els, options, selector) {
        var prop = this._findPropertyByPath_(options.withProperty, options.model),
			eventTypes = 'change';	
    ...
    _bindMethodWithElement_: function ($els, options) {
        var props = this._findPropertiesByPath_(options.dependencies, options.model),
